...
public class CompradorController {
    ...
    private ResponseEntity<EntityModel<CompradorOUTDTO>> comprador(
            MetodoController metodoController, Comprador comprador) {
        return ResponseEntity.ok().body(compradorLink.preparaComprador(comprador, metodoController));
    }
    ...
    public ResponseEntity<EntityModel<CompradorOUTDTO>> porId(@PathVariable String id) {
        return comprador(MetodoController.POR_ID, compradorService.porId(id));
    }
}
